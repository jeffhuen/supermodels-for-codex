...
- Claude task delegation is now gated per-call by Supermodels' own permission broker via Claude Code PreToolUse hooks, replacing the coarse `--allowedTools` allow-list while keeping the full Claude Code task harness. Read-only tasks deny writes and shell; `--write` tasks approve only edits whose canonicalized (symlink-safe) path is inside the workspace; `Bash` is denied in both modes, because a Claude task has no OS sandbox and a shell command would void the path-scoped gating. The broker's settings are the sole permission authority (`--setting-sources ""`), and a missing, crashing, malformed, or timed-out hook denies the call — fail closed, never `bypassPermissions`. Denied calls are recorded in the job's provider events.
