...
If multiple review-capable providers are ready, reviews run all of them in parallel. If only one is ready, reviews use that provider.
...
Direct reviews freeze and revalidate one immutable repository snapshot before provider work begins. Every first pass and cross-challenge receives tools bound to that same snapshot. `get_diff`, `get_review_context`, and `list_changed_files` are losslessly paginated, so the response byte limit is a page size rather than a total-diff cutoff. Conclusive output requires every diff page plus complete `read_file` coverage of every readable/new-side high-risk hunk; pure deletions are proved from the complete diff because deleted source cannot be read from the snapshot tree. Changed-file pages are optional discovery, not acceptance evidence. Native provider truncation is fail-closed, and evidence that was actually attempted but unavailable yields an attributed `inconclusive` result with exact gaps rather than a degraded clean verdict. Present `assume-unchanged` or manual `skip-worktree` entries abort snapshot creation because their working state cannot be proven; absent sparse-checkout entries remain supported.

Git clean filters are executable repository/user configuration, just as when Git itself stages content. Supermodels runs them against a discardable private copy so they cannot change the immutable tree delivered to reviewers, and filtered changed files require complete raw-file inspection. Because a non-invertible filter can make canonical diff lines differ from raw working-file lines, modified or deleted filtered evidence is reported as an exact verification gap rather than being presented as a lossless clean review. Filter programs that deliberately write absolute paths outside their Git worktree remain trusted local code and are outside the plugin's filesystem isolation boundary.
...
The plugin does not embed provider API keys, provider account credentials, or private/confidential OAuth client secrets. Reviews reuse local Claude Code, AGY, and Grok OAuth credentials; AGY refresh uses the public, non-confidential installed-app client metadata used by the CLI family. Claude tokens refresh through Claude Code's OAuth store; Claude subscription/API rate limits are surfaced as provider `rate-limited` results. Claude reviews submit their final structured result via strict tool use and cache the stable request prefix (tool schemas, system persona, preloaded context) across the multi-round review loop; Claude task delegation runs under a read/edit tool allowlist (shell excluded, so `Bash` stays unavailable even if the hook fails) and is gated per-call by Supermodels' own permission broker via Claude Code PreToolUse hooks — in-workspace edits only, and a broken hook denies writes. AGY token refresh uses the same direct OAuth refresh flow as AGY-compatible clients and persists back to the native token store, including rejected-token `401` retries. Grok tokens refresh through the same OIDC flow `grok login` uses (`POST {issuer}/oauth2/token`) and persist back to `~/.grok/auth.json`. Direct reviews receive a shared context packet plus a first immutable diff page and bounded file snippets before the first provider call, but providers still must consume every remaining diff page and make distinct meaningful `read_file` or `search` inspections before final review submission. Antigravity review calls default to Gemini 3.5 Flash High (`gemini-3-flash-preview`) and remain model-led after the evidence gate, with aggregate timeout and cancellation as runaway guards. Code Assist calls use the reference transport pacing defaults and can be tuned with `SUPERMODELS_ANTIGRAVITY_CODE_ASSIST_MODEL`, `SUPERMODELS_ANTIGRAVITY_RPM`, and `SUPERMODELS_ANTIGRAVITY_BURST`. Grok reviews go over a direct OAuth transport to xAI's documented CLI chat proxy (`cli-chat-proxy.grok.com`), default to `grok-4.5` at `high` reasoning effort, and get the most generous review page budget on the panel; an HTTP 426 from the proxy surfaces as an explicit `grok update` error instead of a generic failure. Grok defaults can be tuned with `SUPERMODELS_GROK_MODEL`, `SUPERMODELS_GROK_EFFORT`, `SUPERMODELS_GROK_AUTH_PATH`, and `SUPERMODELS_GROK_RESPONSES_URL`.
...
Antigravity write tasks use the installed `agy` CLI's native default write behavior. Use `--write --provider antigravity` only when that native CLI permission model is acceptable. Grok tasks run over ACP (`grok agent stdio`) by default; read-only tasks are enforced by Supermodels' own permission broker (every write/execute request is denied before the provider can act), while `--write --provider grok` tasks auto-approve the operations Grok requests — including shell commands, not only file edits — at that same broker, with the `GROK_SANDBOX` (`read-only`/`workspace`) environment variable as the actual OS-level containment boundary. `--best-of-n <N>` and `--check` are Grok-exclusive one-shot task modes that run over headless `grok --prompt-file` instead of ACP. `--check` is experimental: current Grok CLI releases (0.2.x) can end a `--check` run as `Cancelled` and drop the final answer (verified live), so Supermodels never appends it automatically and only passes it through on explicit request.
