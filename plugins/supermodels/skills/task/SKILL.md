...
Grok also supports one-shot modes no other provider has, run through headless `grok --prompt-file` instead of ACP: `--best-of-n <N>` runs N attempts and keeps the best one, and `--check` asks Grok to self-verify its output before returning. Known upstream issue: on current Grok CLI releases (0.2.x) the `--check` verifier can end the run as `Cancelled` and drop the final answer, so treat `--check` as experimental and inspect the saved run artifacts if the result looks truncated.
