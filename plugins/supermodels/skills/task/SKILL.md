...
Claude tasks run through the Claude Code CLI with its full task harness. Both read-only and `--write` Claude tasks are gated per-call by Supermodels' own permission broker via Claude Code PreToolUse hooks: read-only tasks deny every write; `--write` tasks approve only edits whose canonicalized (symlink-safe) path stays inside the workspace. `Bash` is denied in both modes — a Claude task has no OS sandbox, so a shell command would void the path-scoped gating — and a missing, crashing, malformed, or timed-out hook fails closed (the call is denied, never allowed by default). Denied calls are recorded in the job's provider events.
