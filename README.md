...
Antigravity reviews use the AGY/Code Assist OAuth credential store and Gemini-style function calling with the same Supermodels-owned read-only repository tools. Supermodels preloads bounded review context before the first Code Assist call, including the diff, changed files, and snippets from changed files, then AGY may request additional read-only tools. Direct review mode defaults to Gemini 3.5 Flash High (`gemini-3-flash-preview`) to avoid Pro quota spikes; native `agy` model aliases are still used for task delegation. Code Assist calls use the reference transport pacing defaults and can be tuned with `SUPERMODELS_ANTIGRAVITY_RPM` and `SUPERMODELS_ANTIGRAVITY_BURST`. Rejected-token `401` responses force direct OAuth refresh before retrying, and macOS keychain read failures do not silently fall back to stale default token files.
